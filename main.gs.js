const TEST_PAYLOAD = JSON.parse(
  '{"uid":"100010126620062","name":"Đỗ Thị Nguyên Phương","timeTaken":"2021-04-01T14:18:02.634Z","postContent":"E cần tìm đầm này ạ,ai có báo em nha","group":"♥️ HỘI THANH LÝ VÁY MAXI, VÁY VOAN HOA, VINTAGE"}'
);
...
function writeToDatabase(PAYLOAD = TEST_PAYLOAD) {
  ...
  DATABASE.getRange(`A${LAST_ROW + 1}:I${LAST_ROW + 1}`).setValues([
    [
      ...
      `=CONCATENATE(A${LAST_ROW + 1}," | ",B${LAST_ROW + 1})`,
    ],
  ]);
}
